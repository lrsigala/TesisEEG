...
##################################### Grafica ###############################################
    ...
    def IniciarSesion(self, checked):
        if self.etapa[1] == False:
            ...
            self.update_speed_ms = int((1/ self.sampling_rate)*1000) #Ajustar a la frecuencia de muestreo
            self.num_points = self.sampling_rate
            self.timer.start(self.update_speed_ms*30)
            print(int((1/ self.sampling_rate)*1000))
            self.etapa[1] = True
            self.__ready()
    ...
    def updategraph(self):
        data = board_shim.get_current_board_data(30)
        for i in range(0, len(self.curves)):
            self.x[i] = self.x[i][30:]  # Remove the first
            ...
            if(self.recording == True):
                self.record[i]=self.record[i]+data[i+1].tolist()
                if(i==0):
                    self.prompts.extend(np.full(len(data[i+1]), vpalabras[self.indexPalabra]))
                    self.state.extend(np.full(len(data[i+1]), vfases[self.index]))
...
#####################################################################################
    ...
    def GuardarSesion(self):
        ...
        self.record=self.record[:16]
        for i in range(0, len(self.record)):
            pass
            print(len(self.record[i]))
        print(f'vector:{len(self.record)}')
